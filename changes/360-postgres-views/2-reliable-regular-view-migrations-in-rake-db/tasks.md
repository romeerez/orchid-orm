...
- [ ] 2.1 Report regular view changes in generated migration output
  - 2.1.1 Add concise create-view and drop-view report lines for regular view AST items using the existing generated migration report naming and color conventions.
  - 2.1.2 Keep view report output focused on the action and view identity, without dumping view SQL or option objects.
...
- [ ] 3.1 Document generated regular-view migration behavior
  - 3.1.1 Clarify that pull/generated regular-view migrations use the same documented `createView` and `dropView` option shape, including nested `with` options, without implying materialized-view or first-class ORM view support.
