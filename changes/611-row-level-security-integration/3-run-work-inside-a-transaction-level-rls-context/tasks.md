...
- [x] 1.5 Cover pqb behavior with tests and verification
  ...
  - 1.5.5 Run `pnpm pqb check -o` for changed pqb tests when focused verification is enough, then run `pnpm pqb check` and `pnpm pqb types` before marking pqb work complete.
...
- [x] 2.1 Expose transaction session options through `$transaction`
  ...
  - 2.1.5 Run `pnpm orm check -o` for changed orm tests when focused verification is enough, then run `pnpm orm check` and `pnpm orm types` before marking orm work complete.
