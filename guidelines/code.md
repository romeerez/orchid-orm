...
- Run the test with `pnpm <pkg> check --silent path/to/file.test.ts`, where `<pkg>` is the package folder name such as `pqb`, `orm`, or `rake-db`. For `schemaConfigs` packages, use `valibot` or `zod`.
...
- Once the new tests and code changes for the task are complete, run all affected tests in the package with `pnpm <pkg> check --silent -o` and fix any failures.
