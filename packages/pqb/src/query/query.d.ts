...
import { PickQueryResult, PickQueryResultReturnType, PickQueryReturnType, PickQuerySelectable, PickQueryShape, PickQueryTable } from './pick-query-types';
...
import { QueryCatch, QueryThen, QueryThenByQuery, QueryThenShallowSimplify, QueryThenShallowSimplifyArr, QueryThenShallowSimplifyOptional } from './then/then';
...
export type SelectableFromShape<Shape extends Column.QueryColumns, Table extends string | undefined> = {
    [K in keyof Shape]: {
        as: K;
        column: Shape[K];
    };
...
export type QueryReturnType = QueryReturnTypeAll | 'one' | 'oneOrThrow' | 'rows' | 'pluck' | 'value' | 'valueOrThrow' | 'void';
...
export interface Query extends IsQuery, PickQueryTable, PickQueryShape, PickQuerySelectable, QueryMethods<unknown> {
    __as: string;
    __hasSelect: boolean;
    __hasWhere: boolean;
...
export declare namespace Query {
    ...
    namespace Order {
        type Arg<T extends OrderNamespace.ArgThis> = OrderNamespace.Arg<T>;
        type Args<T extends OrderNamespace.ArgThis> = OrderNamespace.Args<T>;
    }
    namespace Pick {
...
export type SelectableOfType<T extends PickQuerySelectable, Type> = {
    [K in keyof T['__selectable']]: T['__selectable'][K]['column']['__type'] extends Type | null ? K : never;
}[keyof T['__selectable']];
export type SelectableOrExpressionOfType<T extends PickQuerySelectable, C extends Column.Pick.Type> = SelectableOfType<T, C['__type']> | Expression<Column.Pick.QueryColumnOfType<C['__type'] | null>>;
export declare const queryTypeWithLimitOne: RecordKeyTrue;
...
export type SetQueryReturnsAllResult<T extends PickQueryResult, Result extends Column.QueryColumns> = {
    [K in keyof T]: K extends 'returnType' ? 'all' : K extends 'result' ? Result : K extends 'then' ? QueryThenShallowSimplifyArr<T['result']> : T[K];
} & QueryHasWhere;
export type QueryTakeOptional<T extends PickQueryResultReturnType> = T['returnType'] extends 'value' | 'pluck' | 'void' ? T : T['returnType'] extends 'valueOrThrow' ? {
    [K in keyof T]: K extends 'returnType' ? 'value' : K extends 'then' ? QueryThen<T['result']['value']['__outputType'] | undefined> : T[K];
} : {
...
export type QueryRequire<T extends PickQueryResultReturnType> = T['returnType'] extends QueryReturnTypeAll | 'valueOrThrow' | 'pluck' | 'void' ? T : T['returnType'] extends 'value' ? {
    [K in keyof T]: K extends 'returnType' ? 'valueOrThrow' : K extends 'then' ? QueryThen<Exclude<T['result']['value']['__outputType'], undefined>> : T[K];
} : {
    [K in keyof T]: K extends 'returnType' ? 'oneOrThrow' : K extends 'then' ? QueryThenShallowSimplify<ColumnsShape.Output<T['result']>> : T[K];
};
export type QueryTake<T extends PickQueryResultReturnType> = T['returnType'] extends 'valueOrThrow' | 'pluck' | 'void' ? T : T['returnType'] extends 'value' ? {
    [K in keyof T]: K extends 'returnType' ? 'valueOrThrow' : K extends 'then' ? QueryThen<Exclude<T['result']['value']['__outputType'], undefined>> : T[K];
} : {
    [K in keyof T]: K extends 'returnType' ? 'oneOrThrow' : K extends 'then' ? QueryThenShallowSimplify<ColumnsShape.Output<T['result']>> : T[K];
};
...
export type SetQueryReturnsOneResult<T extends PickQueryResult, Result extends Column.QueryColumns> = {
...
export type SetQueryReturnsPluck<T extends PickQuerySelectable, S extends keyof T['__selectable'] | Expression> = S extends keyof T['__selectable'] ? {
    [K in keyof T]: K extends '__hasSelect' ? true : K extends 'result' ? {
        pluck: T['__selectable'][S]['column'];
    } : K extends 'returnType' ? 'pluck' : K extends 'then' ? QueryThen<T['__selectable'][S]['column']['__outputType'][]> : T[K];
} : {
    [K in keyof T]: K extends '__hasSelect' ? true : K extends 'result' ? {
...
export type SetValueQueryReturnsPluckColumn<T extends PickQueryResult> = {
    ...
    } : K extends 'returnType' ? 'pluck' : K extends 'then' ? QueryThen<T['result']['value']['__outputType'][]> : T[K];
} & QueryHasSelect;
export type SetQueryReturnsPluckColumnResult<T extends PickQueryResult, Result extends Column.QueryColumns> = {
    [K in keyof T]: K extends 'result' ? {
        pluck: T['result']['value'];
    } : K extends 'returnType' ? 'pluck' : K extends 'result' ? Result : K extends 'then' ? QueryThen<T['result']['value']['__outputType'][]> : T[K];
} & QueryHasSelect;
export type SetQueryReturnsValueOrThrow<T extends PickQuerySelectable, Arg extends keyof T['__selectable']> = SetQueryReturnsColumnOrThrow<T, T['__selectable'][Arg]['column']> & T['__selectable'][Arg]['column']['operators'];
export type SetValueQueryReturnsValueOrThrow<T extends PickQueryResult> = {
    [K in keyof T]: K extends 'returnType' ? 'valueOrThrow' : K extends 'then' ? QueryThen<T['result']['value']['__outputType']> : T[K];
};
export type SetQueryReturnsValueOptional<T extends PickQuerySelectable, Arg extends GetStringArg<T>> = SetQueryReturnsColumnOptional<T, {
    [K in keyof T['__selectable'][Arg]['column']]: K extends '__outputType' ? T['__selectable'][Arg]['column'][K] | undefined : T['__selectable'][Arg]['column'][K];
}> & Omit<T['__selectable'][Arg]['column']['operators'], 'equals' | 'not'> & Column.OperatorsNullable<T['__selectable'][Arg]['column']>;
export type SetQueryReturnsColumnOrThrow<T, Column extends Column.Pick.OutputType> = {
    [K in keyof T]: K extends 'result' ? {
        value: Column;
    } : K extends 'returnType' ? 'valueOrThrow' : K extends 'then' ? QueryThen<Column['__outputType']> : T[K];
} & QueryHasSelect;
export type SetQueryReturnsColumnOptional<T, Column extends Column.Pick.OutputType> = {
    [K in keyof T]: K extends 'result' ? {
        value: Column;
    } : K extends 'returnType' ? 'value' : K extends 'then' ? QueryThen<Column['__outputType'] | undefined> : T[K];
} & QueryHasSelect;
export type SetQueryReturnsColumn<T extends PickQueryResult> = {
    [K in keyof T]: K extends 'result' ? {
        value: T['result']['pluck'];
    } : K extends 'returnType' ? 'valueOrThrow' : K extends 'then' ? QueryThen<T['result']['pluck']['__outputType']> : T[K];
} & QueryHasSelect;
export type SetQueryReturnsColumnResult<T extends PickQueryResult, Result extends Column.QueryColumns> = {
    [K in keyof T]: K extends 'result' ? {
        value: T['result']['pluck'];
    } : K extends 'returnType' ? 'valueOrThrow' : K extends 'result' ? Result : K extends 'then' ? QueryThen<Result['pluck']['__outputType']> : T[K];
} & QueryHasSelect;
...
export type SetQueryResult<T extends PickQueryReturnType, Result extends Column.QueryColumns> = {
...
export interface QueryOrExpressionBooleanOrNullResult {
    result: {
        value: Column.Pick.QueryColumnOfType<boolean | null>;
    };
}
export declare const isQueryReturnsAll: (q: Query) => boolean;
export declare const isQuery: (q: unknown) => q is IsQuery;
