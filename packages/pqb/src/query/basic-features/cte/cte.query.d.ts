...
import { PickQueryResult, PickQueryWithDataColumnTypes } from '../../pick-query-types';
...
export interface CteQueryBuilder<T extends PickQueryWithDataColumnTypes> extends Query {
    sql: QuerySql<T['columnTypes']>['sql'];
    relations: EmptyObject;
    withData: T['withData'];
}
export type CteResult<T extends PickQueryWithDataColumnTypes, Name extends string, Q extends PickQueryResult> = {
...
export type CteSqlResult<T extends PickQueryWithDataColumnTypes, Name extends string, Shape extends Column.QueryColumns> = {
    [K in keyof T]: K extends 'withData' ? {
...
export declare const _prependWith: (q: Query, name: string | ((as: string) => void), queryArg: PickQueryResult | ((q: CteQueryBuilder<PickQueryWithDataColumnTypes>) => PickQueryResult)) => Query;
export declare const _with: (q: Query, name: string | ((as: string) => void), queryArg: PickQueryResult | ((q: CteQueryBuilder<PickQueryWithDataColumnTypes>) => PickQueryResult), options?: CteArgsOptions, prepend?: boolean) => Query;
export declare class CteQuery {
    /**
     ...
     * ```
     */
    withSql<T extends PickQueryWithDataColumnTypes, Name extends string, Shape extends Column.Shape.QueryInit>(this: T, name: Name, options: CteOptions, shape: (t: T['columnTypes']) => Shape, expr: (q: T) => Expression): CteSqlResult<T, Name, Shape>;
    withSql<T extends PickQueryWithDataColumnTypes, Name extends string, Shape extends Column.Shape.QueryInit>(this: T, name: Name, shape: (t: T['columnTypes']) => Shape, expr: (q: T) => Expression): CteSqlResult<T, Name, Shape>;
}
