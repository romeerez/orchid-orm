...
import { AfterCommitStandaloneHook, SqlSessionState, TransactionAdapter } from '../../../adapters/adapter';
import { OrchidOrmError } from '../../errors';
import { PickQueryQAndInternal } from '../../pick-query-types';
import { AsyncState } from '../storage/storage';
import { QuerySchema } from '../schema/schema';
export interface AsyncTransactionState extends AsyncState {
    transactionAdapter: TransactionAdapter;
    transactionId: number;
}
export type IsolationLevel = 'SERIALIZABLE' | 'REPEATABLE READ' | 'READ COMMITTED' | 'READ UNCOMMITTED';
...
export type AfterCommitErrorResult = AfterCommitErrorFulfilledResult | AfterCommitErrorRejectedResult;
/**
 ...
 * ```
 */
export declare class AfterCommitError extends OrchidOrmError {
    result: unknown;
    hookResults: AfterCommitErrorResult[];
    constructor(result: unknown, hookResults: AfterCommitErrorResult[]);
}
export type AfterCommitErrorHandler = (error: AfterCommitError) => void | Promise<void>;
export declare const _runAfterCommitHooks: (result: unknown, promises: unknown[], getHookNames: () => string[], catchAfterCommitErrors: AfterCommitErrorHandler[] | undefined) => Promise<void>;
/**
 * Check if inside transaction started by user (not test transaction).
 */
export declare const isInUserTransaction: (trx: AsyncState | undefined) => trx is AsyncState;
