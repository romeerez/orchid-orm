import { PickQueryColumTypes, PickQueryHasSelect, PickQueryMetaSelectableResultRelationsWindowsColumnTypes, PickQuerySelectable, PickQueryShape } from '../pick-query-types';
...
import { AggregateOptions } from './fn-expression';
...
interface QueryReturnsFnAdd<T extends PickQueryColumTypes> extends PickQueryHasSelect {
    type<C extends Column.Pick.QueryColumn>(fn: (types: T['columnTypes']) => C): {
        [K in keyof T]: K extends 'result' ? {
            value: C;
        } : K extends 'returnType' ? 'valueOrThrow' : K extends 'then' ? QueryThen<C['__outputType']> : T[K];
    } & C['operators'];
}
type SetQueryReturnsFn<T extends PickQueryColumTypes, C extends Column.Pick.OutputType> = {
    [K in keyof T]: K extends 'result' ? {
        value: C;
    } : K extends 'returnType' ? 'valueOrThrow' : K extends 'then' ? QueryThen<C['__outputType']> : T[K];
} & QueryReturnsFnAdd<T>;
export declare class QueryExpressions {
    /**
