import { IsSubQuery, QueryRequire, QueryReturnType, QueryTake, QueryTakeOptional, SetQueryReturnsAll, SetQueryReturnsRows, SetQueryReturnsVoid } from './query';
import { ToSQLQuery } from './sql/to-sql';
import { QueryCatchers, QueryThen, QueryThenByQuery, QueryThenShallowSimplify, QueryThenShallowSimplifyArr, QueryThenShallowSimplifyOptional } from './then/then';
...
import { JoinResultRequireMain, QueryJoin } from './basic-features/join/join';
...
import { MergeQuery, MergeQueryArg, MergeQueryMethods } from './extra-features/merge/merge';
...
import { PickQueryResultReturnType, PickQueryRelations, PickQueryResult, PickQueryResultReturnTypeUniqueColumns, PickQuerySelectableShapeAs, PickQuerySelectableShapeRelationsReturnTypeIsSubQuery, PickQueryShapeResultReturnTypeSinglePrimaryKey, PickQueryTableMetaShapeTableAs, PickQueryHasSelectResult } from './pick-query-types';
import { QueryAsMethods } from './basic-features/as/as';
import { QuerySql, Sql } from './sql/sql';
import { SQLQueryArgs } from './db-sql-query';
import { QueryClone } from './basic-features/clone/clone';
...
export interface IsQueryHelperForTable<Table extends string | undefined> extends IsQueryHelper {
    table: Table;
}
export interface QueryHelper<T extends PickQueryTableMetaShapeTableAs, Args extends any[], Result> extends IsQueryHelper {
...
export type QueryHelperResult<T extends IsQueryHelper> = T['__result'];
...
type NarrowInvalidKeys<T extends PickQueryResult, Narrow> = {
    [K in keyof Narrow]: K extends keyof T['result'] ? Narrow[K] extends T['result'][K]['__outputType'] ? never : K : K;
}[keyof Narrow];
interface NarrowValueTypeResult<T extends PickQueryResultReturnType, Narrow> extends Column.QueryColumns {
    value: {
        [K in keyof T['result']['value']]: K extends '__outputType' ? Narrow : T['result']['value'][K];
    };
}
interface NarrowPluckTypeResult<T extends PickQueryResultReturnType, Narrow> extends Column.QueryColumns {
    pluck: {
        [K in keyof T['result']['pluck']]: K extends '__outputType' ? Narrow extends unknown[] ? Narrow[number] : Narrow : T['result']['pluck'][K];
    };
}
type QueryIfResult<T extends PickQueryResultReturnType, R extends PickQueryResult> = {
    [K in keyof T]: K extends 'result' ? {
        [K in keyof T['result'] | keyof R['result']]: K extends keyof T['result'] ? K extends keyof R['result'] ? R['result'][K] | T['result'][K] : T['result'][K] : Column.QueryColumnToOptional<R['result'][K]>;
    } : K extends 'then' ? QueryIfResultThen<T, R> : T[K];
};
export type QueryIfResultThen<T extends PickQueryResultReturnType, R extends PickQueryResult> = T['returnType'] extends undefined | 'all' ? QueryThenShallowSimplifyArr<{
    [K in keyof T['result']]: K extends keyof R['result'] ? T['result'][K]['__outputType'] | R['result'][K]['__outputType'] : T['result'][K]['__outputType'];
} & {
    [K in keyof R['result'] as K extends keyof T['result'] ? never : K]?: R['result'][K]['__outputType'];
...
export declare class QueryMethods<ColumnTypes> {
    ...
    } : `narrowType() error: provided type does not extend the '${NarrowInvalidKeys<T, Narrow> & string}' column type` : (T['returnType'] extends 'pluck' ? Narrow extends unknown[] ? Narrow[number] : Narrow : Narrow) extends (T['returnType'] extends 'pluck' ? T['result']['pluck']['__outputType'] : T['result']['value']['__outputType']) ? {
    ...
    chain<T extends PickQuerySelectableShapeRelationsReturnTypeIsSubQuery, RelName extends keyof T['relations']>(this: T, relName: RelName): T['__subQuery'] extends true | undefined ? [T['returnType'], T['relations'][RelName]['returnsOne']] extends [
        ...
        [K in keyof RelationQueryMaybeSingle<T['relations'][RelName]>]: K extends '__selectable' ? RelationQueryMaybeSingle<T['relations'][RelName]>['__selectable'] & Omit<T['__selectable'], keyof T['shape']> : RelationQueryMaybeSingle<T['relations'][RelName]>[K];
