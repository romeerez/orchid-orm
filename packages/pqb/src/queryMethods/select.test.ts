...
describe('selectMethods', () => {
  ...
  it('should select table.column with aliases', () => {
    ...
    const query = q.select({ aliasedId: 'user.id', aliasedName: 'user.name' });
  ...
  describe('parse columns', () => {
    ...
    it('should parse subquery array columns', async () => {
      ...
      expect((await q.takeOrThrow()).users[0].createdAt instanceof Date).toBe(
        true,
      );
    ...
    it('should parse subquery item columns', async () => {
      ...
      expect((await q.takeOrThrow()).user.createdAt instanceof Date).toBe(true);
