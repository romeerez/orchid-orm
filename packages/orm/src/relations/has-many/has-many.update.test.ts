...
describe('hasMany update', () => {
  ...
  describe('add', () => {
    ...
    it('should add related records in upsert update branch', async () => {
      ...
      const ChatId = await db.chat.get('IdOfChat').create(ChatData);
      const message = await db.message.create({ ...MessageData, ChatId });

      resetQueriesCount();

      await db.user.find(userId).upsert({
        update: { messages: { add: { Id: message.Id } } },
        create: {
          ...UserData,
          messages: {
            create: [{ ...MessageData, ChatId, Text: 'create branch message' }],
          },
        },
      });
      ...
      expect(
        await db.user
          .queryRelated('messages', { Id: userId, UserKey: 'key' })
          .pluck('Id'),
      ).toEqual([message.Id]);
    });
    ...
    it('should disconnect related records in upsert update branch', async () => {
      ...
      const data = await db.user
        ...
        .create({
          ...UserData,
          messages: {
            create: [{ ...MessageData, ChatId, Text: 'disconnect' }],
          },
        });
      ...
      await db.user.find(data.Id).upsert({
        update: { messages: { disconnect: { Text: 'disconnect' } } },
        create: {
          ...UserData,
          messages: {
            create: [{ ...MessageData, ChatId, Text: 'create branch message' }],
      ...
      expect(getQueriesCount()).toBe(1);
      expect(await db.message.find(data.messageIds[0])).toMatchObject({
        AuthorId: null,
      });
      expect(
        await db.user.queryRelated('messages', { Id: data.Id, UserKey: 'key' }),
      ).toEqual([]);
    });
  ...
  describe('set', () => {
    ...
    it('should set related records in upsert update branch', async () => {
      ...
      const userId = await db.user.get('Id').create({
        ...UserData,
        messages: {
          create: [{ ...MessageData, ChatId, Text: 'previous' }],
        },
      });
      await db.message.create({ ...MessageData, ChatId, Text: 'set' });

      resetQueriesCount();

      await db.user.find(userId).upsert({
        update: { messages: { set: { Text: 'set' } } },
        create: {
          ...UserData,
          messages: {
            create: [{ ...MessageData, ChatId, Text: 'create branch message' }],
    ...
    it('should throw when no related records were found by a condition', async () => {
      ...
      expect(result).toEqual({
        err: expect.objectContaining({
          message:
            'Expected to find at least 1 record(s) based on `set` conditions, but found 0',
        }),
      });
    });
  ...
  describe('delete', () => {
    ...
    it('should delete related records in upsert update branch', async () => {
      ...
      const userId = await db.user.get('Id').create({
        ...UserData,
        messages: {
          create: [{ ...MessageData, ChatId, Text: 'delete' }],
        },
      });
      ...
      await db.user.find(userId).upsert({
        update: { messages: { delete: { Text: 'delete' } } },
        create: {
          ...UserData,
          messages: {
            create: [{ ...MessageData, ChatId, Text: 'create branch message' }],
      ...
      expect(
        await db.user.queryRelated('messages', { Id: userId, UserKey: 'key' }),
      ).toEqual([]);
    });
  ...
  describe('update', () => {
    ...
    it('should update related records with distinct data for each condition', async () => {
      ...
      expect(
        await db.user
          ...
          .order('Id')
          .pluck('Text'),
      ).toEqual(['updated 1', 'updated 2', 'message 3']);
    });
    ...
    it('should update related records in upsert update branch', async () => {
      ...
      const userId = await db.user.get('Id').create({
        ...
        messages: {
          create: [
            { ...MessageData, ChatId, Text: 'previous 1' },
            { ...MessageData, ChatId, Text: 'previous 2' },
          ],
        },
      });
      ...
      await db.user.find(userId).upsert({
        update: {
          messages: {
            update: [
              {
                where: { Text: 'previous 1' },
                data: { Text: 'updated 1' },
              },
              {
                where: { Text: 'previous 2' },
                data: { Text: 'updated 2' },
              },
            ],
          },
  ...
  describe('create', () => {
    ...
    it('should create related records in upsert update branch', async () => {
      ...
      await db.user.find(userId).upsert({
        update: {
          messages: {
            create: [
              { ...MessageData, ChatId, Text: 'updated branch message' },
            ],
          },
        },
        create: {
      ...
      expect(
        await db.user
          .queryRelated('messages', { Id: userId, UserKey: 'key' })
          .pluck('Text'),
      ).toEqual(['updated branch message']);
    });
    ...
    it('should create new related records using `on`', async () => {
      ...
      const texts = await db.user.queryRelated('messages', user).order('Text');
  ...
  describe('combined', () => {
    it('should combine all supported nested operations successfully', async () => {
      ...
      const createdMessages = await db.message.createMany([
        { ...MessageData, ChatId: chatId, AuthorId: user1.Id, Text: 'add 1' },
        { ...MessageData, ChatId: chatId, AuthorId: user1.Id, Text: 'add 2' },
        {
          ...MessageData,
          ChatId: chatId,
          AuthorId: user2.Id,
        ...
        { ...MessageData, ChatId: chatId, Text: 'set 1' },
        { ...MessageData, ChatId: chatId, Text: 'set 2' },
      ]);
      const upsertMessage = await db.message.create({
        ...MessageData,
        ChatId: chatId,
        AuthorId: user2.Id,
      ...
      const count = await db.user.find(user2.Id).update({
        messages: {
          ...
          disconnect: [{ Text: 'disconnect 1' }, { Text: 'disconnect 2' }],
          set: [{ Text: 'set 1' }, { Text: 'set 2' }],
          delete: [{ Text: 'delete 1' }, { Text: 'delete 2' }],
          update: [
            { where: { Text: 'add 1' }, data: { Text: 'add 1 updated' } },
            {
              where: { Text: 'disconnect 2' },
              data: { Text: 'disconnect 2 updated' },
            },
            { where: { Text: 'set 1' }, data: { Text: 'set 1 updated' } },
            { where: { Text: 'existing' }, data: { Text: 'existing updated' } },
          ],
          upsert: [
            {
              findBy: { Id: upsertMessage.Id },
            ...
            {
              findBy: { Id: createdMessages[1].Id },
              update: { Text: 'add 2 upserted' },
              create: { ...MessageData, ChatId: chatId },
            },
            {
              findBy: { Id: createdMessages[8].Id },
              update: { Text: 'set 2 upserted' },
              create: { ...MessageData, ChatId: chatId },
            },
            {
              findBy: { Id: createdMessages[2].Id },
              update: { Text: 'disconnect 1 upserted' },
              create: {
                ...MessageData,
                ChatId: chatId,
                Text: 'disconnect 1 upserted',
              },
            },
            {
              findBy: { Id: createdMessages[6].Id },
              update: { Text: 'existing upserted' },
              create: {
                ...MessageData,
                ChatId: chatId,
                Text: 'existing upserted',
              },
            },
          ],
          create: [
            { ...MessageData, ChatId: chatId, Text: 'created 1' },
            { ...MessageData, ChatId: chatId, Text: 'created 2' },
          ],
        },
      });
      ...
      expect(messages).toEqual([
        { Text: 'add 1 updated', AuthorId: user2.Id },
        { Text: 'add 2 upserted', AuthorId: user2.Id },
        { Text: 'created 1', AuthorId: user2.Id },
        { Text: 'created 2', AuthorId: user2.Id },
        { Text: 'disconnect 1', AuthorId: null },
        { Text: 'disconnect 1 upserted', AuthorId: user2.Id },
        { Text: 'disconnect 2', AuthorId: null },
        { Text: 'existing', AuthorId: null },
        { Text: 'existing upserted', AuthorId: user2.Id },
        { Text: 'set 1 updated', AuthorId: user2.Id },
        { Text: 'set 2 upserted', AuthorId: user2.Id },
        { Text: 'upsert: created', AuthorId: user2.Id },
        { Text: 'upsert: not updated', AuthorId: null },
        { Text: 'upsert: updated', AuthorId: user2.Id },
      ]);
    });
