...
Only `grantee` and `schema` are required.
...
1. **Global privileges excluded**: Entries without a `schema`, or with `object` of `'schema'` or `'large_object'` are filtered out (global default privileges are not implemented yet)

### Grouping

Privileges are grouped by the combination of `grantor` + `grantee` + `schema` into a single `DbStructure.DefaultPrivilege` entry.

### Output Structure

Each group produces a `DefaultPrivilege` with:
- `grantor`: from raw data
...
- add `schema` to `deps`: privilege uses a schema
- add `grantor` (if available) and `grantee` to `deps`
