...
import { Db } from 'pqb';
import { asMock, defineTable, sql, testDefaultColumnTypes } from 'test-utils';
...
describe('create and drop table', () => {
  ...
  describe('indexes', () => {
    ...
    it('should create deferrable unique constraints', async () => {
      await testUpAndDown(
        (action) =>
          db[action](
            'table',
        ...
        () =>
          expectSql([
            ...
            toLine(`
              ALTER TABLE "table"
                ADD CONSTRAINT "table_email_key"
                UNIQUE ("email")
                DEFERRABLE INITIALLY IMMEDIATE
            `),
            toLine(`
              ALTER TABLE "table"
                ADD CONSTRAINT "table_position_key"
                UNIQUE NULLS NOT DISTINCT ("list_id", "position")
    ...
    it('should create deferrable unique constraints with constraint options', async () => {
      await testUpAndDown(
        (action) =>
          db[action](
            'table',
          ...
          expectSql([
            ...
            toLine(`
              ALTER TABLE "table"
                ...
                INCLUDE ("deleted_at")
                WITH (fillfactor = 70)
                USING INDEX TABLESPACE fast_tablespace
                DEFERRABLE INITIALLY DEFERRED
            `),
          ]),
      );
      ...
      await testUpAndDown(
        ...
        () =>
          expectSql([
            ...
            `,
            `CREATE UNIQUE INDEX "table_email_idx" ON "table" ("email")`,
          ]),
      );
    });
    ...
    it('should reject deferrable unique constraints with index-only options', async () => {
      ...
      await expect(
        db.createTable('table', { noPrimaryKey: true }, (t) => ({
          email: t.text().unique({
            name: 'table_email_key',
            deferrable: 'deferred',
      ...
      await expect(
        db.createTable(
          ...
          (t) =>
            t.index([{ expression: 'lower("email")' }], {
              name: 'table_email_key',
              unique: true,
              deferrable: 'deferred',
              using: 'btree',
        ...
        'Deferrable unique constraints do not support index option: using',
      );
      ...
      await expect(
        db.createTable(
          ...
          (t) =>
            t.unique([{ column: 'email', order: 'DESC' }], {
              name: 'table_email_key',
              deferrable: 'deferred',
            }),
        ),
      ).rejects.toThrow(
        'Deferrable unique constraints do not support index column option: order',
      );
    });
  });
  ...
  describe('timestamps', () => {
    ...
    it('should use timestamp default from the defineTable option', async () => {
      await testUpAndDown(
        async (action) => {
          db.options.baseTable = undefined;
          db.options.defineTable = {
          ...
          try {
            ...
            db.options.defineTable = undefined;
          }
        },
        () =>
          expectSql(`
  ...
  describe('foreign key', () => {
    ...
    it('should handle column foreign key to a function-style table reference', async () => {
      ...
      await testUpAndDown(
        (action) =>
          db[action]('table', (t) => ({
            iD: t.identity().primaryKey(),
            foreignId: t.integer().foreignKey(() => ForeignTable, 'id'),
          })),
        () =>
          expectSql(`
            CREATE TABLE "table" (
              "i_d" int4 GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
              "foreign_id" int4 NOT NULL REFERENCES "schema"."foreignTable"("id")
            )
          `),
      );
    });
    ...
    it('should handle table foreign key to a function-style table reference', async () => {
      const ForeignTable = defineTable(
        'foreignTable',
        { schema: 'schema' },
        (t) => ({
      ...
      await testUpAndDown(
        (action) =>
          db[action](
            ...
            (t) =>
              t.foreignKey(['iD', 'naMe'], () => ForeignTable, [
                'foreignId',
                'foreignName',
              ]),
          ),
        () => {
          expectSql(`
            CREATE TABLE "table" (
              ...
              "na_me" text NOT NULL,
              ${toLine(`
                CONSTRAINT "table_i_d_na_me_fkey"
                  FOREIGN KEY ("i_d", "na_me")
                  REFERENCES "schema"."foreignTable"("foreign_id", "foreign_name")
              `)}
            )
          `);
        },
