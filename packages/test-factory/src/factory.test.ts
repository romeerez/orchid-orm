...
describe('factory', () => {
      ...
      const config: FactoryConfig = {
        fakeDataForTypes: {
          jsonb(c) {
            const result =
              'json keys: ' +
              Object.keys((c.inputSchema as ZodObject<ZodRawShape>).shape).join(
                ', ',
              );
            return () => result;
          },
  ...
  describe('pick with create (no argument)', () => {
    it('should only generate values for picked columns in the async branch', async () => {
      // This test verifies the fix for a bug where the else branch in processCreateData
      // iterated over `fns` (all factory functions) instead of `pick` (filtered keys).
      // The bug caused identity columns to be generated even when they should be
      // auto-generated by the database.
      
      const pickedFactory = factory.user.pick({ name: true, age: true });
      
      // When calling create() without arguments, only picked columns should be generated.
      // The identity column (id) should NOT be in the generated data before insert.
      const user = await pickedFactory.create();
      
      // The user should have an id (from database), but only name and age should
      // have been generated by the factory
